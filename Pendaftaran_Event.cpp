...
bool cekDuplikasiUser(const string& nama, const string& nim, 
                      User dataUser[], int total);
bool cekNamaPemainDalamTeam(Team data[], int total, string nim, 
                            string namaPemain);
bool cekNamaTeamUntukUser(Team data[], int total, string nim, 
                          string namaTeam);
...
string inputNamaTeam(const string& pesan) {
    ...
    while (true) {
        ...
        if (nama.empty()) {
            cout << "Error: Nama team tidak boleh kosong!\n";
            continue;
        }
...
string getNamaEventByID(int eventID) {
    for (int i = 0; i < totalEvent; i++) {
        if (daftarEvent[i].id == eventID) {
            return daftarEvent[i].nama;
        }
    }
...
int hitungTeamPerEvent(string namaEvent) {
    ...
    for (int i = 0; i < totalTeam; i++) {
        if (getNamaEventByID(daftarTeam[i].eventID) == namaEvent) {
            count++;
        }
    }
    return count;
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        for (int j = 0; j < n - i - 1; j++) {
            if (data[j].nama > data[j + 1].nama) {
                ...
                int tp = jumlahPeserta[j]; 
                jumlahPeserta[j] = jumlahPeserta[j + 1]; 
                jumlahPeserta[j + 1] = tp;
                swapped = true;
            }
        }
        if (!swapped) break;
...
void quickSortHadiahDesc(Event data[], int jumlahPeserta[], 
                         int low, int high) {
    if (low < high) {
...
void menuSorting(Event data[], int n, int jumlahPeserta[]) {
    ...
    do {
        cout << "\n=== MENU SORTING EVENT ===\n";
        cout << "1. Sort Berdasarkan Nama Event (A-Z)\n";
        cout << "2. Sort Berdasarkan Hadiah Event (Terbesar)\n";
        cout << "3. Kembali\nPilihan: ";
        pilihan = inputAngka("");
...
int binarySearchID(Event data[], int n, int cariID) {
    ...
    while (low <= high) {
        ...
        else if (tempData[mid].id < cariID) {
            low = mid + 1;
        }
        else {
            high = mid - 1;
...
int cariTeamByUser(Team data[], int total, string nim) {
    for (int i = 0; i < total; i++) {
        if (data[i].pemilikNIM == nim) return i;
    }
    return -1;
}
...
void cetakBarisEvent(string id, string nama, string lokasi, 
                     string hadiah, string tanggal, 
                     string peraturan, string team) {
...
void tampilEvent(Event data[], int n, int idx) {
    ...
    cetakBarisEvent(to_string(data[idx].id), 
                   data[idx].nama.substr(0, 18), 
                   data[idx].lokasi.substr(0, 13),
                   data[idx].hadiah, 
                   data[idx].tanggal, 
                   perChunks[0], 
                   teamStr);
    
    for (size_t i = 1; i < perChunks.size(); i++) {
        cetakBarisEvent("", "", "", "", "", perChunks[i], "");
    }
    cetakGarisEvent();
...
void lihatTeamDanAnggota() {
    ...
    cout << right << setw(14) << "Jumlah Anggota" << " |\n";
    cout << "+------------------------------+------+";
    cout << "------------------------+----------------+\n";
    ...
    for (int i = 0; i < totalTeam; i++) {
        ...
        cout << "| " << left << setw(28) 
             << daftarTeam[i].namaTeam.substr(0, 28) << " | ";
        cout << right << setw(4) << daftarTeam[i].id << " | ";
        cout << left << setw(22) 
             << namaEventSekarang.substr(0, 22) << " | ";
        ...
        if (daftarTeam[i].jumlahAnggota > 0) {
            ...
            for (int j = 0; j < daftarTeam[i].jumlahAnggota; j++) {
                if (j > 0) semua += ", "; 
                semua += daftarTeam[i].anggota[j].nama;
            }
            ...
            while (pos < (int)semua.length()) {
                ...
                if (end < (int)semua.length()) {
                    ...
                    if (koma != string::npos && koma <= pos + MAX_L) {
                        end = koma + 1;
                    }
                }
                string bagian = semua.substr(pos, end - pos);
                while (!bagian.empty() && 
                      (bagian.front() == ' ' || bagian.front() == ',')) {
                    bagian.erase(0, 1);
                }
                while (!bagian.empty() && 
                      (bagian.back() == ' ' || bagian.back() == ',')) {
                    bagian.pop_back();
                }
                if (!bagian.empty()) {
...
// ======================== CRUD TEAM (USER) ========================
void buatTeam(Team data[], int &total, User dataUser[], int uLogin, 
              Event dataEvent[], int totalEvent) {
    if (total >= 100) { 
    ...
    if (cekNamaTeamUntukUser(data, total, dataUser[uLogin].nim, 
                             namaTeamBaru)) {
        cout << "Error: Anda sudah memiliki team dengan nama '" 
             << namaTeamBaru << "'!\n";
        return;
    }
    ...
    while (t.jumlahAnggota < 10) {
        ...
        if (inv) { 
            cout << "Nama pemain hanya boleh huruf dan spasi!\n"; 
            continue; 
        }
        
        if (cekNamaPemainDalamTeam(data, total, dataUser[uLogin].nim, pl)) {
            cout << "Error: Nama pemain '" << pl 
                 << "' sudah ada dalam team Anda!\n";
            continue;
        }
...
void tambahPemain(Team data[], int total, string nim) {
    ...
    string namaBaru = inputNamaPemain("");
    
    if (cekNamaPemainDalamTeam(data, total, nim, namaBaru)) {
        cout << "Error: Nama pemain '" << namaBaru 
             << "' sudah ada dalam team Anda!\n";
        return;
    }
...
void lihatPeraturanDanTeam(Team dataTeam[], int totalTeam, 
                           User dataUser[], int uLogin, 
                           Event dataEvent[], int totalEvent) {
    ...
    if (idx != -1) {
        ...
        for (int i = 0; i < totalEvent; i++) {
            if (dataEvent[i].nama == namaEventSekarang && 
                namaEventSekarang != "⚠️ Event Dihapus") {
                cout << "\nPeraturan: " << namaEventSekarang << "\n";
                cout << (dataEvent[i].peraturan.empty() ? 
                        "Belum ada peraturan.\n" : 
                        dataEvent[i].peraturan + "\n");
                break;
            }
        }
        if (namaEventSekarang == "⚠️ Event Dihapus") {
    ...
    for (int i = 0; i < totalTeam; i++) {
        ...
        cout << (i + 1) << ". " << dataTeam[i].namaTeam 
             << " - " << namaEvt 
             << " (" << dataTeam[i].jumlahAnggota << " anggota)\n";
    }
    cout << "----------------------------------------\n";
}
...
void updatePemain(Team data[], int total, string nim) {
    int idx = cariTeamByUser(data, total, nim);
    if (idx == -1 || data[idx].jumlahAnggota == 0) { 
        cout << (idx == -1 ? "Anda tidak memiliki team.\n" : 
...
void menuKelolaTeam(Team dataTeam[], int &totalTeam, User dataUser[], 
                    int uLogin, Event dataEvent[], int totalEvent) {
    int p;
    do {
        cout << "\n=== MENU KELOLA TEAM ===\n";
        ...
        if (p == 1) {
            buatTeam(dataTeam, totalTeam, dataUser, uLogin, 
                     dataEvent, totalEvent);
        }
        else if (p == 2) {
            lihatPeraturanDanTeam(dataTeam, totalTeam, dataUser, 
                                  uLogin, dataEvent, totalEvent);
        }
        else if (p == 3) {
...
void hapusEvent(Event data[], int &n) {
    ...
    cout << "Konfirmasi: Hapus event '" << data[idx].nama 
         << "'? (y/n): "; 
    string konfirmasi; 
    ...
    for (int t = totalTeam - 1; t >= 0; t--) {
        if (daftarTeam[t].eventID == id) {
            for (int k = t; k < totalTeam - 1; k++) {
                daftarTeam[k] = daftarTeam[k + 1];
            }
            totalTeam--;
        }
    }
    ...
    cout << "Team yang terdaftar di event ini juga telah dihapus.\n";
}
...
void menuUser() {
    ...
    do {
        ...
        if (p == 1) {
            ...
            menuKelolaTeam(daftarTeam, totalTeam, daftarUser, 
                           UserLogin, daftarEvent, totalEvent);
        }
        else if (p != 3) {
            cout << "Pilihan tidak valid. Masukkan angka 1-3.\n";
        }
...
int main() {
    ...
    daftarEvent[totalEvent++] = {1, "Lomba Lari", "Lapangan", 
                                 "1.000.000,00", "10/06/2026", 
                                 "Dilarang pakai alas kaki"};
